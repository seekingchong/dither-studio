...
test('符号：Lime Circuit / Checkmate / PETSCII Glitch / Acid Cipher / Ink Atlas 预设用上新符号，选择器里能挑到它们', async ({ page }) => {
  await page.goto('/');
  await dropImage(page);
  await page.getByRole('tab', { name: '符号' }).click();
  const rows = page.locator('[data-testid="glyph-levels"] .glyph-level');
  ...
  const picker = page.getByTestId('glyph-picker');
  await expect(picker).toBeVisible();
  await expect(picker.locator('[data-glyph]')).toHaveCount(64);
  ...
  await expect(atlasPicker.locator('[data-glyph]')).toHaveCount(64);
