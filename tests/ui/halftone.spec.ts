...
test('网点页签：自己的预设与分节，来回切换不丢参数，保存与历史按风格分开，SVG 出真网点', async ({ page }) => {
  ...
  await expect(tabs).toHaveText(['抖动', '排线', '网点', '符号', '历史']);
...
test('网点：Yellow Pop 的涟漪扰动参数随扰动种类显隐，摘要带扰动', async ({ page }) => {
  await page.goto('/');
  await dropImage(page);
  await page.getByRole('tab', { name: '网点' }).click();
  // 形状里已没有「符号」——按明暗换符号的活归「符号」页签
  await page.locator('[data-param="halftone.shape"]').click();
  await expect(page.getByRole('option', { name: '符号', exact: true })).toHaveCount(0);
  await expect(page.getByRole('option', { name: '十字', exact: true })).toHaveCount(1);
  await page.keyboard.press('Escape');
  // Yellow Pop 排在折叠线以下，先展开卡片；Symbol Sketch / Typewriter 已不在这一页
