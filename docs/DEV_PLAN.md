...
| 符号预设：Pixel Blocks（2026-09） | 已完成：按第十张参考图给「符号」风格加一套预设 Pixel Blocks（`glyph-pixel-blocks`）——米纸上橙蓝双色的几何海报，形状按明暗排成一片像素状的构成，顺带把图里缺的两种元素收进符号库（75 → 77 种）。新符号：密竖纹 `bars`（五道贯穿格子的细竖线，栅距是竖纹与竖栅的一半，邻格接成一片更密的条纹）、小方 `pixel`（不到一半大的实心小方块，参考图里满是这种像素点）。两个都由现成图元拼成——密竖纹是五条跨格线段 `S`，小方是实心方 `q` 的小号版，没有新图元；老符号编码不变，新的 2 个接在后面，墨量、选择器分组、解读文案自动跟上，没有新的推荐序列。预设是 8 阶自定义序列 空 → 小点 → 小方 → 三角 → 圆点 → 棋盘 → 密竖纹 → 实心方：亮处零星的黑小点，中间调橙色的小方、三角、圆点，暗处蓝的棋盘格、密竖纹到成片的实心方。分级配色把三种墨分给各阶（近黑 `#1B1712` 两阶 → 橙 `#F26A1B` 三阶 → 蓝 `#6C79C1` 两阶 → 深蓝 `#4E5CAE` 一阶），米纸 `#EBE7DD` 不反相；16px 方格、符号大小 100%（实心方在邻格之间只留一道纸色缝、密竖纹与棋盘格接得上，暗部读成一整块像素砖）、亮部缩小 55%、交界混合 60%（橙蓝掺在一起，暗块里也落几颗橙方，边缘是参考图那种台阶状的锯齿）、点缀 5%、亮度 +8 / 对比 +12 让纸留得住；特效栈预置一枚胶片颗粒（强度 18、1px、单色）当纸纹。参考图是排版好的静态海报，这里把同一套形状与油墨接到明暗上，任何素材都能出这个味道。单测 `glyph.test.ts` 加 4 个（新符号墨量关系、渲染接缝、SVG 图元、预设本身），`state.test.ts` 符号预设数 18 → 19，UI 用例 `glyph.spec.ts` 加 1 个，选择器符号数 75 → 77 |
