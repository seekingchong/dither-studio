...
| 符号预设：Acid Cipher（2026-09） | 已完成：按第三张参考图给「符号」风格加一套预设——荧光柠檬绿底上的墨色「密文」：亮处一粒小点、短斜线、十字，中间调圆圈与叉号，暗处靶心、圈十、圈叉，像终端屏幕上一行行等宽字符。参考图本身是横放的，转正之后短斜线正是库里的「短斜线」方向，图里的八种记号符号库原本都有，所以这次不加新符号。预设 Acid Cipher（`glyph-cipher`）是 8 阶自定义序列 小点 `pip` → 短斜线 `slashshort` → 十字 `plus` → 圆圈 `ring` → 叉号 `xmark` → 靶心 `ringdot` → 圈十 `circleplus` → 圈叉 `circlex`，按墨量 10% → 14% → 21% → 26% → 26% → 33% → 38% → 39% 单调递增（暗部三阶的墨量接近，靠 ◎ / ⊕ / ⊗ 三种形状的交替出纹理，不用实心块压死暗部，画面保持参考图那种通透）；12px 方格、符号 74%、亮部缩小 30%（小点真的小，圆圈与圈叉接近等大，像等宽字符）、线粗 12%、交界混合 45%（阶与阶之间像信号噪点一样过渡）、不点缀；统一色 墨绿黑 `#0E150A` 配柠檬绿纸 `#C9F52C`，对比度 +20，浅底深符号不反相。参数是用引擎离线渲染人像与几何样例反复比出来的：更小的格子或更大的符号都显得密而重，实心点收尾会丢掉通透感。单测 `glyph.test.ts` 加 1 个（序列、颜色、亮部缩小比例、渐变图左墨右纸），`state.test.ts` 符号预设数改 14，UI 用例 `glyph.spec.ts` 在 Lime Circuit / Checkmate 那条里加上它；README 同步 |
