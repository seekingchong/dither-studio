...
| 符号预设：Swiss Mosaic（2026-09） | 已完成：按第十张参考图给「符号」风格加第 19 套预设 Swiss Mosaic（`glyph-swiss`），并把图里的方块语言收进符号库（75 → 79 种）。参考图是瑞士风格的双色方块海报——整幅画就是一张方格网：暗处的格子填满、与邻格连成整片实色（亮一档的方块落在里面就成了纸色的洞），往亮处先退成留缝的方块，再缩成三分之一格的小方，最后散成零星几粒落在纸上。新符号四个，都在「几何」组，靠已有的 `b` 图元拼成，引擎没动：把 `square` 那块方当成 3×3 的小方格，像素方 `pixel`（只留中心一格）、像素阶 `pixstair`（对角三格）、像素十字 `pixcross`（十字五格，药房十字的呼应）、像素回 `pixframe`（围一圈八格，中心留纸）——小方边长正好是大方的三分之一、彼此相接，外缘与 `square` 齐平，墨量就是实心方的 1/9、3/9、5/9、8/9；最暗一阶直接用竖栅那套带来的实心格 `block`（跨格填满，不随符号大小缩放）。顺带加了第 9 套推荐序列「马赛克」`mosaic`：空 → 像素方 → 像素阶 → 像素十字 → 棋盘 → 像素回 → 实心方 → 实心格，全是方块、按墨量排好。预设是 7 阶自定义序列 空 → 像素方 → 像素十字 → 实心方 ×3 → 实心格，20px 方格、符号大小 90%（留出格线一样的纸缝，实心格不受它影响）、亮部缩小 20%；颜色只有绿与蓝两支，分级配色交错——浅绿 `#4FD08F` / `#35C182` → 绿 `#00A05B` → 蓝 `#0F5FC4` → 绿 `#00A05B` → 蓝 `#0F5FC4` → 深蓝 `#0B4FA8`，第 5 阶回到绿，交界混合 85% 把相邻两阶搅在一起，成片的蓝里就掺进绿方块、绿的一片里也落几粒蓝；淡灰纸 `#EFEFEF`、亮底不反相、对比 +20。参考图是两色纯平的方块，这里多给了一档浅绿与一档深蓝、亮部还会退成小方与十字，过渡从「有 / 无」变成「整片 → 留缝 → 小方 → 无」，散开的那一段更细腻。单测 `glyph.test.ts` 加 5 个（编码与墨量次序、3×3 小格的定位与外缘、SVG 的 `<rect>` 个数、「马赛克」序列、预设本身），`state.test.ts` 符号预设数 18 → 19，`help.ts` 补「马赛克」的选项解读，UI 用例 `glyph.spec.ts` 补一段 |
