...
| 符号预设：Desync（2026-09） | 已完成：按参考图三给「符号」风格加第 16 套预设 Desync（`glyph-desync`）——白纸上的「失步」海报：主体被拆成横向扫描线与黑色像素块，荧光黄绿的像素散在暗部，几行整体错位像信号丢帧。没有新符号，全部用现有图元：6 阶自定义序列 空 → 短横 `minus` → 横线 `dash` → 双横 `equals` → 实心方 `square` → 实心方 `square`，分级配色前四阶与最暗一阶墨黑 `#111111`、第五阶荧光黄绿 `#D6FF1A`，纸色暖白 `#F2F2EE`，不反相，对比度 +20。黄绿夹在双横与黑块之间、交界混合 70%，被打散成暗部里的碎片而不是一整片色带。符号大小 130% 配亮部缩小 45%——短横那一阶缩到 83% 留出缝像点线，实心方的两阶 ≥ 118% 正好盖满 10px 的格子、相邻块之间不留线（实心方的边是格子的 85%，要 118% 才盖满）；线粗 22%。特效栈一条「扫描行位移」：带高 10px 与格子等高、行比例 6%、最大位移 36px，整行的方块一起挪、不切碎符号；画布半高是间距整数倍时（默认 1000×600 就是）位移带与格子行对齐。单测 `glyph.test.ts` 加 1 个（序列 / 配色 / 大小关系 / 特效栈 / 渐变渲染左黑右白中间有黄绿），`state.test.ts` 符号预设计数 13 → 14，UI 用例 `glyph.spec.ts` 加 1 个 |
| 符号预设：Riso Blocks（2026-09） | 已完成：按新的参考图给「符号」风格加一套预设 Riso Blocks（`glyph-riso-blocks`）——米纸上橙蓝双色的丝网 / riso 海报，几何形状按明暗排成一片像素状的构成，顺带把图里缺的元素收进符号库（62 → 66 种）。新符号：小方 `pixel`（不到一半大的实心小方块，参考图里满是这种像素点）、密竖纹 `bars`（五道贯穿格子的细竖线，比原来的竖纹密一倍，邻格接成一片等距条纹）、横砖 `hbrick` / 竖砖 `vbrick`（一边占满格子与邻格接成带、另一边只有三分之一格宽的实心块）。引擎：`glyphs.ts` 图元加了 `B`——横纵半边各给一个的实心矩形，距离场直接用 `shapes.ts` 里导出的 `box`，SVG 出 `<rect>`；小方复用实心方的 `q`，密竖纹复用跨格线段 `S`。老符号编码不变，新的 4 个接在后面；墨量、选择器分组、解读文案自动跟上，没有新的推荐序列。预设是 8 阶自定义序列 空 → 小点 → 小方 → 三角 → 圆点 → 棋盘 → 密竖纹 → 实心方：亮处零星的黑小点，中间调橙色的小方、三角、圆点，暗处蓝的棋盘格、密竖纹到成片的实心方。分级配色把三种墨分给各阶（近黑 `#1B1712` 两阶 → 橙 `#F26A1B` 三阶 → 蓝 `#6C79C1` 两阶 → 深蓝 `#4E5CAE` 一阶），米纸 `#EBE7DD` 不反相；16px 方格、符号大小 100%（实心方在邻格之间只留一道纸色缝、密竖纹与棋盘格接得上，暗部读成一整块像素砖）、亮部缩小 55%、交界混合 60%（橙蓝掺在一起，暗块里也落几颗橙方，边缘是参考图那种台阶状的锯齿）、点缀 5%、亮度 +8 / 对比 +12 让纸留得住；特效栈预置一枚胶片颗粒（强度 18、1px、单色）当纸纹。参考图是排版好的静态海报，这里把同一套形状与油墨接到明暗上，任何素材都能出这个味道。单测 `glyph.test.ts` 加 4 个（新符号墨量关系、渲染接缝、SVG 图元、预设本身），`state.test.ts` 符号预设数 16 → 17，UI 用例 `glyph.spec.ts` 加 1 个，选择器符号数 62 → 66 |
