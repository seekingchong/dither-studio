...
| 符号预设：Ink Atlas（2026-09） | 已完成：按第六张参考图给「符号」风格加一套预设 Ink Atlas（`glyph-atlas`）——暖白纸 `#FAF7F1` 上墨蓝 `#16202D` 的符号图谱，7 阶自定义序列 空 → 小点 → 斜线 → 十字 → 实心三角 → 圆点 → 实心方，全用现成符号，符号库没动。关键是符号大小与亮部缩小的配合：大小开到 118%，实心方的半边是 0.85r，1.18 × 0.85 > 1 让最暗一阶抵到格子边、成片的黑连起来，格间只剩一道发丝细的纸色缝，黑块上就浮出一层网格（参考图里那层格线）；同一个 118% 会让圆点糊成一片，所以亮部缩小拉满到 90%——最暗的方块仍是 118%，圆点落到 100%（刚好一格宽、邻格相切不粘连），三角 83%、十字 65%、小点 30%（13px 格子里只剩两三像素），六种符号的层级一眼分得开。斜线是跨格图元，长度只跟格子走、不受大小影响，邻格接成长斜线；13px 方格、线粗 12%、交界混合 58% 让相邻两阶掺在一起，边界像手排的一样散开；点缀 5% 撒些圆圈与三角框，对比 +18。参考图是纯白配纯黑，这里换成暖白纸与墨蓝，同样的黑白关系读起来更像印品。单测 `glyph.test.ts` 加 1 个（序列、逐阶大小的比例关系、渲染后暗端几乎铺满墨但仍留着格线），`state.test.ts` 符号预设数 14 → 15，UI 用例 `glyph.spec.ts` 补一段 |
