...
| 符号预设：Shape System（2026-09） | 已完成：按参考图（「Geometric System / Color Edition」海报）给「符号」风格加一套预设 Shape System（`glyph-system`）——奶白纸上一套平涂的几何色块，8 阶自定义序列 空 → 小方点 → 圆点 → 实心三角 → 杉树 → 小屋 → 横板 → 实心方。图里的元素符号库里还差四种，一并补齐（75 → 79）：小方点 `tinysquare`（不到一半大的实心小方，比小点还少墨）、竖板 `pillar`（横板转 90°，上下邻格接成一条）、杉树 `fir`（上小下大两层三角，两层之间收一道腰）、小屋 `hut`（三角屋顶压在方身上，屋檐两侧探出）；都用现成的图元拼——矩形 `r`（横板那一家带进来的）与多边形 `P`，引擎没加新图元。老符号的编码不变，新的 4 个接在后面，墨量、选择器分组与解读文案自动跟上，没有新的推荐序列。预设的三处要点：亮部缩小 62% 拉开大小差（最亮一阶不到最暗一阶的一半，同一张画里既有小方点又有整块的色块）；符号大小 110% 让暗部的方块之间只剩一道纸色细缝、横板左右接成长条（实心方的边长是符号的 85%，100% 时缝太宽）；交界混合 50% 把相邻两阶掺在一起，边界像手摆出来的而不是等高线，再加点缀 5% 让成片的色块里偶尔漏出一个空心圆或三角框。配色是参考图那套里索纸色：橙 `#EF6C1F`（第 2、4 阶）、粉 `#F3BAD0`（第 3 阶）、墨绿 `#17643F`（杉树）、蓝 `#4A79CE`（小屋与横板）、砖红 `#9C6161`（最暗一阶）配奶白纸 `#EDE9E2`，第 1 阶留白、配色跟第 2 阶同色，色板上不出现一块纸色；22px 方格、浅纸深墨不反相、对比 +10，预设里带一层「胶片颗粒」12% 出纸纹。单测 `glyph.test.ts` 加 4 个（四个新符号的墨量关系、竖板的跨格接缝与小方点 / 杉树 / 小屋的轮廓、SVG 图元、预设本身：序列、配色、大小关系、特效栈，渲染后暗端墨量是亮端 5 倍以上且粉 / 绿 / 蓝 / 砖红都在），`state.test.ts` 符号预设数 18 → 19，UI 用例 `glyph.spec.ts` 新加一条（符号选择器 75 → 79） |
