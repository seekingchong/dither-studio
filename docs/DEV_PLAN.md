...
| 符号预设：Ultramarine Bitmap（2026-09） | 已完成：按第六张参考图给「符号」风格加一套预设 Ultramarine Bitmap（`glyph-ultramarine`）——淡紫白纸上只有一族群青的位图海报，像早期屏幕的有序抖动：8 阶自定义序列 空 → 小点 → 四点 → 十字 → 叉号 → 棋盘 → 密网 → 实心方，暗处大片实心块，往亮里依次是密网、棋盘格，再往亮是零星的叉、十字、四点与小点，全用现成符号，符号库没动。11px 方格、符号大小 100%（实心块、密网与棋盘格在邻格之间接上，棋盘格才拼得成格纹）、线粗 18%、亮部不缩小、点缀关掉（大片实心里不掺圈和三角）；交界混合开到 60%，明暗交界处两阶的符号掺在一起，边界碎成参考图那样的噪点。前五套符号预设里深底反相的居多，这套是浅纸深墨、不反相：淡紫白纸 `#F0EEF7`、对比 +15。参考图是纯平涂的一种蓝，这里分级配色仍收在同一族群青里，只按阶从亮处偏紫的 `#4B42F2` 走到暗处的深群青 `#120C86`，大片暗部因此有厚度而不是一块死蓝。单测 `glyph.test.ts` 加 1 个（序列、同族群青逐阶变深、渲染后暗端近乎实心且墨都是蓝的），`state.test.ts` 符号预设数 14 → 15，UI 用例 `glyph.spec.ts` 补一段 |
