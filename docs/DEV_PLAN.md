...
| 网点暗部反白 / Pixel Dots 预设（2026-09） | 已完成：网点风格新增「暗部反白」一组参数（`halftone.holes` / `holeStart` / `holeSize` / `field`，共 4 条，网点参数 19 → 23），参考日历海报那种像素画点阵——亮处黑点由小到大，墨量过了起点的格子翻成黑底挖白孔：黑底是 field 倍大的同形状（默认 150%，≥ 142% 四角闭合），相邻的连成一片圆鼓边的黑区，白孔从 holeSize 起按响应曲线越暗越小、全黑处消失；增益与分级先作用在墨量上，两段共用。引擎：`geometry.ts` 加 `coverageToDot`（每格 `{ size, hole }`，`HalftoneScreen.hole` 只在反白开着时分配）；`render.ts` 的 `distanceAt` 做距离场减法 `max(d, −dHole)`，孔连融合长出的桥一起挖，CMYK 每层各挖各的；`svg.ts` 每张带孔的网格出一个 `<mask>`（白底黑孔，`maskUnits=userSpaceOnUse`，范围取画布对角线一半再多留一圈格子），`countDots` 把孔也算进去。内置网点预设加「Pixel Dots」（黑白、16px 方格圆点、面积正比、最小网点 12%、反白起点 50% / 白孔 55% / 黑底 150%、对比 +15），网点预设 11 → 12。单测：`coverageToDot` 两段与分级、光栅的孔 / 黑底 / 四角缝 / 融合桥、SVG mask 与 CMYK 单层 mask |
