...
| 符号预设：Swiss Mosaic（2026-09） | 已完成：按第六张参考图给「符号」风格加一套预设 Swiss Mosaic（`glyph-swiss`），并把图里的方块语言收进符号库（62 → 67 种）。参考图是瑞士风格的双色方块海报——整幅画就是一张方格网，暗处的格子填满、与邻格连成整片实色（亮一档的格子落在里面就成了纸色的洞），往亮处先退成留缝的方块，再缩成三分之一格的小方，最后散成零星几粒落在纸上。新符号五种，都在「几何」组：像素方 `pixel`（把 `square` 那块方切成 3×3，只留中心一格）、像素阶 `pixstair`（对角三格）、像素十字 `pixcross`（十字五格，药房十字的呼应）、像素回 `pixframe`（围一圈八格，中心留纸）、满格方 `block`（新图元 `B`：按格子而不是符号半径出距离，两头各多出半像素，和贯穿的线一样跨格，成片的暗部连成一整块）；前四种由已有的 `b` 图元拼成，墨量按小方个数递增，SVG 直接出 `<rect>`。顺带加了第 9 套推荐序列「马赛克」`mosaic`：空 → 像素方 → 像素阶 → 像素十字 → 棋盘 → 像素回 → 实心方 → 满格方，全是方块、按墨量排好。预设本身是 7 阶自定义序列 空 → 像素方 → 像素十字 → 实心方 ×3 → 满格方，20px 方格、符号大小 90%（留出格线一样的纸缝，满格方不受它影响）、亮部缩小 20%；颜色只有绿与蓝两支，分级配色交错——浅绿 `#4FD08F` / `#35C182` → 绿 `#00A05B` → 蓝 `#0F5FC4` → 绿 `#00A05B` → 蓝 `#0F5FC4` → 深蓝 `#0B4FA8`，第 5 阶回到绿，交界混合 85% 把相邻两阶搅在一起，成片的蓝里就掺进绿方块、绿的一片里也落几粒蓝；淡灰纸 `#EFEFEF`、亮底不反相、对比 +20。参考图是两色纯平的方块，这里多给了一档浅绿与一档深蓝、亮部还会退成小方与十字，过渡从「有 / 无」变成「整片 → 留缝 → 小方 → 无」，散开的那一段更细腻。单测 `glyph.test.ts` 加 4 个（新符号的编码与墨量次序、3×3 小格的定位与外缘、满格方跨格且不随符号大小缩放、「马赛克」序列），`state.test.ts` 符号预设数 14 → 15，`help.ts` 补「马赛克」的选项解读，UI 用例 `glyph.spec.ts` 补一段（预设、选择器 67 种、序列切到「马赛克」） |
