...
| 符号预设：Ultramarine Bitmap（2026-09） | 已完成：按参考图给「符号」风格加一套预设 Ultramarine Bitmap（`glyph-ultramarine`），顺带把参考图那种有序抖动的密度阶补全（79 → 83 种）。参考图是淡紫白纸上只有一种群青的位图海报，灰阶不靠线条疏密、靠成块的方格：新符号 角块 `quarter`（左上一个象限铺满，¼）与 缺角块 `trio`（四个象限铺满三个，¾），和已有的 棋盘 `checker`（² ⁄ ₄）、实心格 `block`（4/4）连成一族——填的象限层层包含，越暗只是多墨、不挪位置，一路从稀疏点阵铺到整片实底；亮处那几阶补了 方点 `speck`（不到一半大的实心小方块，位图上的一颗像素）与 小叉 `xdot`（比「叉号」再小一圈、不出格的叉），配上已有的 小圈 `ringtiny`，一格一颗、彼此不连，就是参考图上零星撒开的小记号。引擎：象限块要和邻格接得严丝合缝、又不能跟着「符号大小」缩，正好是竖栅那个跨格图元 `V` 的两轴版——给 `V` 加上可选的 `y` / `h`（纵向缺省仍是贯穿整格的竖带，竖栅与实心格照旧），距离场同时换成矩形的精确有符号距离（原来的 `max(dx, dy)` 在四角外侧偏小；竖带的纵向在格内恒为负，所以竖栅那几个符号画出来不变），SVG 出同一个 `<rect>`。预设本身：8 阶自定义序列 空 → 方点 → 小叉 → 小圈 → 角块 → 棋盘 → 缺角块 → 实心格（墨量逐阶递增），11px 方格、符号大小 100%、线粗 18%、亮部不缩小、点缀关掉（大片实底里不掺圈和三角）；交界混合 70%，明暗交界处两阶的符号掺在一起，边界碎成参考图那样的噪点。前面几套符号预设深底反相的居多，这套是浅纸深墨、不反相：淡紫白纸 `#F0EEF7`、对比 +15。参考图是纯平涂的一种蓝，这里分级配色仍收在同一族群青里，只按阶从亮处偏紫的 `#4B42F2` 走到暗处的深群青 `#120C86`，大片暗部因此有厚度而不是一块死蓝。单测 `glyph.test.ts` 加 4 个（新符号的墨量阶梯、角块只占一个象限 / 缺角块只缺左下 / 格内接缝是实墨 / 两个块不随大小缩放、SVG 图元、预设本身），`state.test.ts` 符号预设数 18 → 19，UI 用例 `glyph.spec.ts` 在「按参考图做的几套预设」那条里补一段 |
