...
| 符号预设：Raster Spectrum（2026-09） | 已完成：按第六张参考图给「符号」风格加一套预设 Raster Spectrum（`glyph-raster`）——黑底上竖排光栅的海报，整幅画由疏到密的竖线拼成，顺带把这一路竖条纹收进符号库（62 → 66 种）。新符号 虚竖线 `bardash`（半格长的竖线段，上下邻格断开、接成一条虚线）、双竖纹 `stripe2`（两道贯穿竖线，间距半格）、密竖纹 `stripe4`（四道贯穿竖线，间距四分之一格）、竖带 `band`（上下铺满整格、左右留缝的实心竖带），与原有的 竖线 `bar`、竖纹 `stripes` 凑成一整条「一格里几道竖线」的墨量阶梯，间距一律按格宽等分，邻格接上后整片等距。引擎图元加一种 `V`（跨格实心竖带：横向半宽按 r 算所以随符号大小走，纵向直接铺满 spanY，上下邻格接成一整条；SVG 出 `<rect>`）；老符号编码不变，新的 4 个接在最后，墨量、选择器分组、解读文案自动跟上，没有新的推荐序列。预设本身是 7 阶自定义序列 空 → 虚竖线 → 竖线 → 双竖纹 → 竖纹 → 密竖纹 → 竖带，9×6 的扁格子让竖线细、横向分层粗，线粗 20%、符号大小 125%（只影响竖带的宽度，跨格线段不随大小变）让最亮一阶的竖带占到格宽七成、留一条细缝；分级配色从藏蓝 `#123A7A`、亮蓝、草绿、青 `#63DEFF` 走到米白 `#FAF7EA`，最亮的竖带留给淡黄 `#F2EC9A`——成片之后就是参考图左边那种发光的淡黄大色域；交界混合 38%，近黑纸 `#05080D`、反相、对比 +30，再叠一层 14% 的胶片颗粒压住大片纯黑。单测 `glyph.test.ts` 加 2 个（新符号的墨量阶梯与渲染接缝、这套预设），`state.test.ts` 符号预设数 14 → 15，UI 用例 `glyph.spec.ts` 补一段 |
