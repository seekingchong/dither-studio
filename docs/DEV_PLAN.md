...
| 符号预设：Ink Atlas（2026-09） | 已完成：按第七张参考图给「符号」风格加一套预设 Ink Atlas（`glyph-atlas`）——暖白纸 `#FAF7F1` 上墨蓝 `#16202D` 的符号图谱，7 阶自定义序列 空 → 小点 → 断斜线 → 十字 → 实心三角 → 圆点 → 圆角方；参考图里的两样东西现成符号顶不了，符号库补了 2 种（62 → 64）：断斜线 `slashdash`（跨格斜线两头各收 22%，邻格之间留下断口——现成的 `slash` 成片时会接成一条不断头的长线，读起来像排线而不是图例；长度只跟格子走、不受符号大小影响）、圆角方 `roundsquare`（实心方磨圆四角，新图元 `r` 复用网点的圆角方距离场，SVG 出带 rx 的实心 `<rect>`）。关键是符号大小与亮部缩小的配合：大小开到 122%，圆角方的半边是 0.85r，1.22 × 0.85 > 1 让最暗一阶抵过格子边、成片的黑连起来，格间只剩一道发丝细的纸色缝（参考图里那层格线），而四个圆角在格点上又凑出一小块纸色星芒——远看是整块黑，近看比参考图那种硬邦邦的方块讲究；同一个 122% 会让圆点糊成一片，所以亮部缩小拉满到 90%——最暗的圆角方仍是 122%，圆点落到 104%（刚好一格宽、邻格相切不粘连），三角 85%、十字 67%、小点 31%（13px 格子里只剩两三像素），六种符号的层级一眼分得开。13px 方格、线粗 12%、交界混合 58% 让相邻两阶掺在一起，边界像手排的一样散开；点缀 5% 撒些圆圈与三角框，对比 +18。参考图是纯白配纯黑，这里换成暖白纸与墨蓝，同样的黑白关系读起来更像印品。单测 `glyph.test.ts` 加 4 个（新符号的墨量关系、断斜线在格点断开且不随大小变、圆角方铺满时格点留星芒、SVG 图元、预设的序列与逐阶大小比例），`state.test.ts` 符号预设数 15 → 16，UI 用例 `glyph.spec.ts` 补一段（选择器 62 → 64） |
