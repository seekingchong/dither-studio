...
| 符号预设：Swiss Mosaic（2026-09） | 已完成：按第六张参考图给「符号」风格加一套预设 Swiss Mosaic（`glyph-swiss`）——瑞士海报式的双色方块马赛克：淡灰纸 `#EFEFEF` 上一整片方格，暗处方块连成实块，往亮处逐格缩小、碎开，最后散成零星几粒落在纸上。7 阶自定义序列 空 → 棋盘 → 实心方（往下四阶都是实心方，靠「亮部缩小」拉出大小梯度），最亮一阶留白让稀疏区真的空出来，第 2 阶用棋盘（对角两个小方块）把边缘碎得更细；符号大小 90% 留出格线一样的纸缝，20px 方格。颜色只有绿与蓝两支，靠分级配色交错——亮到暗是 浅绿 `#4FD08F` / `#35C182` → 绿 `#00A05B` → 蓝 `#0F5FC4` → 绿 `#00A05B` → 蓝 `#0F5FC4` → 深蓝 `#0B4FA8`，第 5 阶回到绿，再把交界混合开到 85%，成片的蓝里就掺进绿方块、绿的一片里也落几粒蓝，像参考图那样两色交错；最暗一阶的深蓝给实块留一点厚度。亮底不反相，对比 +20。参考图是两色纯平的方块，这里多给了一档浅绿与一档深蓝、亮部方块还会缩小，过渡从「有 / 无」变成「大 → 小 → 无」，散开的那一段更细腻。全用现成符号，符号库与引擎没动。单测 `glyph.test.ts` 加 1 个（序列、配色、亮底不反相、渐变两端的疏密与中间调两色都在），`state.test.ts` 符号预设数 14 → 15，UI 用例 `glyph.spec.ts` 补一段 |
