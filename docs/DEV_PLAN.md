...
| 符号预设：Aperture Grille（2026-09） | 已完成：按第六张参考图给「符号」风格加一套预设 Aperture Grille（`glyph-grille`）——黑底上一幅假彩色的扫描图，整幅盖着一层等距的竖线栅，像隔着显像管的荫罩栅拍下来的：亮处栅线又粗又亮，暗处细到几乎看不见。这次分阶不靠换形状，而靠「同一个栅距下越来越粗的竖带」，所以符号库加了一家六个（62 → 68 种）：竖栅·微 / 细 / 中 / 粗 / 满 `grillehair` `grillefine` `grillemid` `grillebold` `grillefull`——都是三道贯穿格子的实心竖带，中心固定在格宽的三等分点上（与竖纹同一个栅距），只有带宽在变，墨量 15% / 33% / 51% / 69% / 87% 五档等差，缝始终等宽，邻格接上后整幅画面是一片栅距不变、只有粗细在变的竖栅；再加实心格 `block`，填满整格、邻格拼成一整片。引擎：图元多一种跨格竖带 `V`（`{ x, w }` 都以半格为单位，上下贯穿整格，距离场是横向带子与纵向整格取交，SVG 出实心 `<rect>`）——与线段 `S` 的区别是粗细写死在符号里、不跟「线粗」「符号大小」走，细带也不会被抗锯齿冲淡成灰，分级配色才够艳。预设本身是 7 阶自定义序列 留空 → 竖栅微 / 细 / 中 / 粗 / 满 → 实心格，16px 方格、交界混合 45%，分级配色 深靛 `#0E2B57` → 蓝 `#12579F` → 亮蓝 `#0F9AD8` → 青绿 `#27D2BE` → 柠檬黄 `#D6E84A` → 暖白 `#FFF6E2` 配近黑纸 `#04070C`、反相、对比 +15；另外关掉「线性空间」（`tone.linear: false`）——线性灰度下大半画面会挤在最亮那一阶，关掉后 7 个阶才均匀铺在明暗上。参考图里的栅线是照片上的摩尔纹，这里按明暗分阶排开、栅距对齐格子，画面读起来更整。单测 `glyph.test.ts` 加 5 个（编码与墨量等差、栅距不随粗细变且上下贯穿、粗细不跟大小 / 线粗走、SVG 出实心 `<rect>`、预设本身），`state.test.ts` 符号预设数 14 → 15，UI 用例 `glyph.spec.ts` 补一段、符号选择器 62 → 68 |
