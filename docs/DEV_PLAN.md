...
| 符号预设：PETSCII Glitch（2026-09） | 已完成：按第五张参考图给「符号」风格加一套预设 PETSCII Glitch（`glyph-petscii`）——黑底上青与荧光绿两色的字符画，像 C64 的 PETSCII：8 阶自定义序列 空 → 小点 → 双点 → T → H → M → 棋盘 → 密网，暗处零星的小点与冒号、中间调字母、亮处拼成棋盘格与密网，全用现成符号，符号库没动。两种颜色靠分级配色逐阶交替（暗青 `#24B7C4` → 暗绿 `#A3DD3F` → 青 `#4DEBFF` / 绿 `#C6FF4A` 交替），交界混合开到 80%，同一片区域里两色符号就掺在一起，青绿交错但不乱；11px 方格、符号大小 100%（棋盘格与密网在邻格之间接上）、线粗 16%、亮部不缩小，近黑纸 `#070A0C`、反相、对比 +20。参考图里是密密麻麻随机撒的字符，这里按明暗分阶排开、两色按阶交替，画面读起来更整。单测 `glyph.test.ts` 加 1 个（序列、配色、渲染后亮部两色都在），`state.test.ts` 符号预设数 13 → 14，UI 用例 `glyph.spec.ts` 补一段 |
