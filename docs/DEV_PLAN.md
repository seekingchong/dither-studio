...
| 符号预设：Ultramarine Bitmap（2026-09） | 已完成：按第七张参考图给「符号」风格加一套预设 Ultramarine Bitmap（`glyph-ultramarine`），顺带给符号库补上参考图那种有序抖动的密度阶（62 → 68 种）。参考图是淡紫白纸上只有一种群青的位图海报，灰阶不是靠线条疏密而是靠**成块的方格**：新符号 角块 `quarter`（左上一个象限铺满，¼）、缺角块 `trio`（四个象限铺满三个，¾）、满格块 `block`（整格铺满，4/4），与已有的 棋盘 `checker`（² ⁄ ₄）连成一族——填的象限层层包含，越暗只是多墨、不挪位置，一路从稀疏点阵铺到整片实底；亮处那几阶补了 方点 `speck`（不到一半大的实心小方块，位图上的一颗像素）、小圈 `ringsmall`（比「圆圈」小一圈的空心小圆）、小叉 `xdot`（不出格的小叉），一格一颗、彼此不连，就是参考图上零星撒开的小记号。引擎：图元新增 `B` 跨格实心方——中心与半边都以半格为 1（跟贯穿线段 `S` 一样两头多出半像素），所以它按格铺、不随 `glyph.size` 缩放，邻格的块接得严丝合缝，满格块铺出来是一整片实底、接缝上不留半墨的发丝线；距离场用轴对齐矩形的 SDF（横纵各按自己的半格算，长方格上也铺得满），SVG 出同一个 `<rect>`。预设本身：8 阶自定义序列 空 → 方点 → 小圈 → 小叉 → 角块 → 棋盘 → 缺角块 → 满格块，11px 方格、符号大小 100%、线粗 18%、亮部不缩小、点缀关掉（大片实底里不掺圈和三角）；交界混合 70%，明暗交界处两阶的符号掺在一起，边界碎成参考图那样的噪点。前几套符号预设深底反相的居多，这套是浅纸深墨、不反相：淡紫白纸 `#F0EEF7`、对比 +15。参考图是纯平涂的一种蓝，这里分级配色仍收在同一族群青里，只按阶从亮处偏紫的 `#4B42F2` 走到暗处的深群青 `#120C86`，大片暗部因此有厚度而不是一块死蓝。单测 `glyph.test.ts` 加 4 个（新符号的墨量阶梯、满格块整片无接缝 / 角块只占一个象限 / 缺角块只缺左下、三个块不随大小缩放、SVG 图元、预设本身），`state.test.ts` 符号预设数 15 → 16，UI 用例 `glyph.spec.ts` 补一段（阶梯表、色板、选择器里 68 个符号与新符号的分组） |
