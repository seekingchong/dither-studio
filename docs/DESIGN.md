...
- **三种风格的颗粒粗细都叫「像素尺寸」**：排线 / 网点在数据上是横向、纵向两个间距，面板上合成一个「像素尺寸」滑杆（同时写两个值，拖动合并成一条撤销记录）加一个「横纵分开」开关；开关打开才换成横、纵各自的滑杆，关掉时纵向同步成横向，方案本身横纵不等时自动打开。合成控件的 `data-param` 是 `hatch.cell` / `screen.cell`，开关是 `*.split`（`ui/panel/sections.ts` 的 `CELL_PAIRS`）。
