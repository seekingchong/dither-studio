...
- **导出不在左栏**：整个应用只有一个导出按钮，在预览头「画布」菜单右侧，按当前坑位的媒体类型换文案与去处——图片是「导出图片」（直接存 PNG），视频 / GIF 是「导出视频」（开导出对话框）。左栏操作行只剩页签与「设置」。
...
- 分节标题行高 44px，整行可点，hover `control-subtle`；标题 16/26 SemiBold，摘要 14/22 tertiary 右对齐、超长省略。节与节之间 1px 描边分隔，首节不画线。
