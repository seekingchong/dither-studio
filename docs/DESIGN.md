...
预览缩到 100% 以下时，画布的后备存储只开屏幕物理像素那么大，帧照浏览器 / GPU 显示大图的方式（mipmap 逐级平均 + 双线性）
缩进去（`image-rendering: auto`）——这就是浏览器 / Figma 显示一张导出图时做的事，所以预览里看到的糊就是导出图真的会有的糊；100% 及以上后备存储是画布尺寸，
`image-rendering: pixelated` 让每个像素都是实的。原理与参数选法见 `docs/PREVIEW_SCALING.md`。

素材卡片底下有一条编辑条，「原图」「结果」两个页签都在：旋转 90°、左右 / 上下镜像、画面裁剪缩放（1×–4×）。
改的是素材本身、成品跟着变，所以对着成品调也用同一条，不用来回切页签。
缩放始终等比——放大多少就等比裁掉多少，画面比例不变；放大后直接拖预览挪裁剪窗口，两页都能拖。镜像作用在旋转之后的画面上，
跟看到的方向一致。这些变换在送进流水线之前就烤进源帧，所以结果预览、PNG 与视频导出都自动带上；换素材即清空。

视频素材卡片底下还多一条裁剪条，与编辑条一样两个页签都在——裁的是哪一段，对着源画面挑与对着成品挑都要顺手：拖两端定裁剪范围、拖窗口中间整体挪，
按在窗口外面就把窗口整个居中挪过去。窗长自己定，最短 0.1 秒、最长整段；刚载入的视频默认取开头 4 秒（短于 4 秒的就是整段）。
键盘上三段各管各的：起点、终点、窗口整体各是一个 `slider`，方向键 0.1 秒 / Shift 1 秒微调，Home / End 顶到各自的极限；
拖过谁，焦点就落在谁身上，松手接着用方向键微调。改的过程一直在给反馈：标题上的「裁剪 X.X 秒」与右边的起止时间跟着数，
窗口宽度就是这一段占整段的比例，正在拖的那一端加粗；暂停时拖两端画面当场跟到被拖的那一头（拖起点看起点那一帧、拖终点看终点那一帧；
「原图」页看到的是源画面，「结果」页看到的是同一帧的成品），播放中则不抢画面——播放本来就在新窗口里循环。
两个页签共用同一份窗口状态，在哪边拖都写回同一处。预览在这个窗口里循环，顶部进度条的 min / max 也跟着窗口走，导出视频只出这一段
（导出帧率 × 窗长，裁 2 秒按 60 fps 就是 120 帧、按 30 fps 就是 60 帧）。

边线附近按下算「拖这一端」是按位置判的（`targetAt`，边线左右 9px），不看压在最上面的是哪个元素：
窗口被拖到最短、两端的把手叠在一起时，中间仍然是「整体挪」，两端从窗口外面够得着。
...
（设计稿 node 45:3247，98.958×59.375，圆角 7.125）——放在那儿的不是预览画面，而是真正导出的成品：
图片走「导出图片」同一条路导成 PNG，视频 / GIF 走「导出视频」同一条路导成 60 fps 的 H.264 MP4
（没有 H.264 编码器时 VP9 / VP8 WebM，视频只导「原图」页裁出来的那一段），整个文件送进预览窗口，
用 `<img>` / `<video>` 显示（`object-fit: contain`，缩小交给浏览器），所以看到的就是最终导出文件放进界面里的样子，
视频自己循环播，跟主窗口播到哪一帧无关。

导出在主窗口里做（`ui/interface-preview/exporter.ts`）：开着预览窗口期间盯着参数、素材、素材编辑、裁剪窗口，
一变就重导一份送过去——图片认最近一帧全分辨率渲染结果（新帧一到就重编 PNG），视频改动后等 500ms 再开始、
在途的那次立刻作废；视频用自己克隆的 `<video>` 逐帧定位，主窗口的播放不受影响；成品按输入键缓存，关窗再开、
输入没变就直接给。视频逐帧编码时预览窗口底部有一条状态条报进度，封面容器底边也有一条细线；导出失败时留着上一份成品、
只把原因显示出来。

两扇窗在 Electron 里是 file:// 的不透明源，互相读不到对方的 DOM，成品只能靠 `postMessage` 传
（字节是 `ArrayBuffer`，可转移对象，过去零拷贝，主窗口的缓存里留自己的一份）。预览窗口就绪时说一声「要成品」，
...
- **导出视频的选项**：对话框里四项——质量（中 / 高 / 超高码率）、帧率（30 / 60，默认 60）、分辨率（「按倍率」0.25×–4× 或「自定义」直接写尺寸）。
  分辨率**始终等比**：自定义那档只认一个数，写宽度就按画布比例推高度、写高度就推宽度，两个框永远是同一个比例。
  放大不是把成品图拉大——那只会糊——而是把画布连同「画布上的尺寸」（像素格、间距、笔画粗细、特效里标了 px 的子参数）
  按同一比例一起放大后原样重渲一遍：格子数一个不变，图案与预览里看到的一致，只是每个格子占的像素更多、边缘更实
  （`ui/export/resolution.ts`，与预览降分辨率 `scaleParamsForPreview` 是同一套想法）。选项下面那行小字实时报最终尺寸、帧率与大概多少帧。
...
- **四种风格的颗粒粗细都叫「像素尺寸」**：排线 / 网点 / 符号在数据上是横向、纵向两个间距，面板上合成一个「像素尺寸」滑杆（同时写两个值，拖动合并成一条撤销记录）加一个「横纵分开」开关；开关打开才换成横、纵各自的滑杆，关掉时纵向同步成横向，方案本身横纵不等时自动打开。合成控件的 `data-param` 是 `hatch.cell` / `screen.cell` / `tile.cell`，开关是 `*.split`（`ui/panel/sections.ts` 的 `CELL_PAIRS`）。
- **符号风格的「符号」分节是一张阶梯表**（`ui/panel/GlyphLevelsEditor.tsx`）：从亮到暗一行一阶，每行是 这一阶代表明暗的灰色块 · 「第 N 阶」（首尾标最亮 / 最暗）· 形状按钮（与下拉同框同高，左边是符号缩略图 `GlyphIcon`，直接用引擎的 SVG 图元画）· 颜色色块。点形状弹出符号选择器（`GlyphPicker`，固定定位的弹层，整个符号库按 点 / 线 / 几何 / 字符 分四组、组内按墨量从少到多排，选中的墨色描边），点色块弹出与色板同一个取色层。每一阶的形状 / 颜色在 schema 里标了 `custom`，不在参数栅格里单独出现；改任一阶的形状，序列下拉自动变成「自定义」，统一色下改某一阶的颜色自动转成「分级配色」（弹层里有一行提示）。
...
左栏操作行的页签是「抖动」「排线」「网点」「符号」「历史」：前四个是艺术风格，点它等于切 `style.type`；风格页签下方是该风格的预设卡片与参数分节。
