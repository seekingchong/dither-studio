...
- 符号：同样的网格，但用格子里画的**形状**区分明暗，不是点的大小——画面按明暗均分成 2–8 阶，每一阶配一种符号和一种颜色。符号库 64 种（点：空 / 小点 / 圆点 / 双点 / 四点 / 圆圈 / 靶心 / 双圈 / 四叶 / 花；线：短竖 / 短横 / 横线 / 竖线 / 斜线 / 反斜线 / 短斜线 / 断斜线 / 双横 / 双竖 / 十字 / 叉 / 叉号 / 星号 / 八芒 / 网格 / 密网 / 点线 / 折线 / 竖纹；几何：实心与空心的三角 / 方 / 菱、圆角方、倒三角 / 六边形 / 六边框 / 圆角框 / 框点 / 叉框 / 田 / 圈叉 / 圈十 / 棋盘 / 城堡；字符：1 4 6 7 8 Ø % T L V Z N M H E Y），每种都算过在默认大小下盖住格子的墨量；8 套推荐序列（草图 / 打字机 / 线格 / 记号 / 几何 / 字符 / 字母 / 点阵）各 8 个符号按墨量排好，选了几阶就等距抽几个——最亮的阶配墨最少的符号，最暗的配墨最多的；「符号」分节里一行一阶，点形状在按墨量分组排序的选择器里换任一阶（序列即转为自定义），点色块改那一阶的颜色；符号大小、亮部缩小（亮的阶用更小的符号）、线粗按格子比例、交界混合、点缀圆圈三角、种子；网格与网点同一套（横纵中心距、角度、方格 / 交错、偏移、扰动），各存各的；颜色统一色 / 分级配色（每阶一色）/ 原图色；抗锯齿，SVG 出线段与描边图元
...
- 体验：内置预设 49 个（抖动 11、排线 9：Hatching、Pencil、Engraving、Crosshatch、Woodcut、Rain、Beads、Blueprint Lines、Brick；网点 13：默认、Poster、Newsprint、Comic、CMYK Print、Ink Blob、Line Screen、Line Flow、Mosaic、Triangles、Color Dots、Honeycomb、Yellow Pop（涟漪扰动）；符号 16：默认、Symbol Sketch、Typewriter、Terminal、Line Mesh、Geo Poster、Neon Levels、Confetti、Letterpress、Ripple Type、Blueprint Marks、Lime Circuit（黑底荧光绿的电路板符号）、Checkmate（墨绿底的棋盘格、花、折线、竖纹与城堡，每阶一色）、PETSCII Glitch（黑底青绿两色的 C64 字符画：小点、冒号、字母到棋盘格与密网，两色逐阶交替）、Acid Cipher（柠檬绿底的墨色密文：小点、短斜线、十字、圆圈、叉号到靶心、圈十、圈叉）、Ink Atlas（暖白纸配墨蓝的符号图谱：小点、断斜线、十字、三角、圆点到圆角方，暗处的黑块之间留出一道细网格、格点上留出纸色星芒））、用户预设（按风格分页显示，「历史」页都在）、撤销重做与快捷键（切风格页签也可撤销）、浅 / 深主题、GPU（WebGL2）开关；默认方案是有序 Bayer 2×2 单色，每种风格记住上次用的方案
