...
- 界面：无应用内顶栏，macOS 上系统标题栏只留三个圆点、下面一条分隔线；左右分栏可拖拽、中间一条分隔线，左栏参数栅格按栏宽在一排三 / 二 / 一之间自适应；参数分节可折叠、收起时显示当前值摘要；画布尺寸 / 适配 / 缩放集中在预览区右上角的「画布」菜单，唯一的导出按钮在它右侧（图片是「导出图片」存 PNG，视频 / GIF 是「导出视频」开对话框，左栏不再放导出按钮）；打开、复制当前帧 PNG、撤销、重做只走快捷键（⌘/Ctrl + O / C / Z / ⇧Z，左栏「设置」里有一览），面板上不放这些按钮
