...
- 体验：内置预设 10 个、用户预设、浅 / 深主题、GPU（WebGL2）开关；默认方案是有序 Bayer 2×2 单色
- 界面：左栏参数按锚点式分区排成一列（点 tab 定位），画布尺寸 / 适配 / 缩放集中在预览区右上角的「画布」菜单；打开、复制当前帧 PNG、撤销、重做只走快捷键（⌘/Ctrl + O / C / Z / ⇧Z，设置菜单里有一览），面板上不放按钮
