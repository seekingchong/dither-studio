...
- 界面：无应用内顶栏，macOS 上系统标题栏只留三个圆点；左右分栏可拖拽，左栏参数栅格按栏宽在一排三 / 二 / 一之间自适应；参数分节可折叠、收起时显示当前值摘要；画布尺寸 / 适配 / 缩放集中在预览区右上角的「画布」菜单，「导出视频」在它右侧；打开、复制当前帧 PNG、撤销、重做只走快捷键（⌘/Ctrl + O / C / Z / ⇧Z，左栏「设置」里有一览），面板上不放这些按钮
...
`-x64.dmg`（Intel）两个安装包。**推到 `main` 分支、推 `release/**` 分支或打 `v*` tag 都会触发**，
所以每次合并进 main 都会有一份对应的安装包，不会再出现"装的包里没有最新改动"的情况。
