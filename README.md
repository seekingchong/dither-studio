...
- 特效栈：扫描线 CRT、胶片颗粒、JPEG 损坏、块位移、扫描行位移、像素排序、波形、桶形、像素散射、灰度块描边（把画面按明暗分成 2–16 阶，同一阶连成的色块沿格子边缘描线：粗细、颜色、圆角 / 方角可调，每一阶的描边可单独开关，可只留描边不留画面；添加时阶数跟随当前风格的灰阶数，线贴着抖动 / 排线 / 网点 / 符号各自的格子边而不是噪点边），可堆叠排序
