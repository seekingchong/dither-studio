...
- 网点：规则网格上按明暗放大缩小的矢量点。形状 8 种（圆 / 方 / 圆角方 / 菱形 / 三角 / 六边形 / 线条 / 十字）、网点大小与最小网点、面积或直径正比的响应、网点增益、灰阶分级（2–32 档）、点融合（距离场 smooth-min，单独的小点不会被吃掉）、抗锯齿开关；网格横向 / 纵向中心距、角度、方格或交错排列、偏移、网格扰动（涟漪 / 波浪 / 流动 / 随机，把每颗点从格心推开，规则网格成了被水波推歪的网；强度以格为单位、波长、种子，采样跟着点走）；颜色双色（网点色 + 背景色）/ 原图色 / CMYK 四层分色叠印（青 15° 品 75° 黄 0° 黑 45°）；边缘抗锯齿，CPU 渲染
...
- 导出：PNG、SVG（当前帧的矢量版：抖动把同色块合并成 path，排线出真正的笔画——每档一个圆角矩形定义、格子上 `<use>` 摆到笔心，连线另出一条 path；网点每颗点就是一个 circle / rect / polygon，网格旋转写在 g transform 上，融合用滤镜近似）、60 fps 视频
...
- 体验：内置预设 45 个（抖动 11、排线 9：Hatching、Pencil、Engraving、Crosshatch、Woodcut、Rain、Beads、Blueprint Lines、Brick；网点 12：默认、Poster、Newsprint、Comic、CMYK Print、Ink Blob、Line Screen、Mosaic、Triangles、Color Dots、Honeycomb、Yellow Pop（涟漪扰动）；符号 13：默认、Symbol Sketch、Typewriter、Terminal、Line Mesh、Geo Poster、Neon Levels、Confetti、Letterpress、Ripple Type、Blueprint Marks、Lime Circuit（黑底荧光绿的电路板符号）、Checkmate（墨绿底的棋盘格、花、折线、竖纹与城堡，每阶一色））、用户预设（按风格分页显示，「历史」页都在）、撤销重做与快捷键（切风格页签也可撤销）、浅 / 深主题、GPU（WebGL2）开关；默认方案是有序 Bayer 2×2 单色，每种风格记住上次用的方案
