...
- 特效栈：扫描线 CRT、胶片颗粒、JPEG 损坏、块位移、扫描行位移、像素排序、波形、桶形、像素散射、叠加原图（把原图 / 原视频当前帧垫在成品下面当背景：正片叠底 / 滤色 / 正常三种叠法，可调不透明度、饱和度、亮度、对比度、高斯模糊开关与半径、尺寸缩放、横纵偏移；四种风格共用）、灰度块描边（把画面按明暗分成 2–16 阶，同一阶连成的色块沿格子边缘描线：粗细、颜色、圆角 / 方角可调，每一阶的描边可单独开关，可只留描边不留画面；添加时阶数跟随当前风格的灰阶数，线贴着抖动 / 排线 / 网点 / 符号各自的格子边而不是噪点边），可堆叠排序
