...
- 风格：抖动与排线（Hatching）两种，左栏页签「抖动 | 排线 | 历史」切换；影调调整、强制背景、特效、画布两边共用
...
- 艺术风格：左栏三个平级的风格页签——「抖动」「排线」「网点」，各有自己的预设与参数分节，影调 / 画布 / 特效共用；切页签就是切风格，各风格的参数都留着
...
- 网点：规则网格上按明暗放大缩小的矢量点。形状 9 种（圆 / 方 / 圆角方 / 菱形 / 三角 / 六边形 / 线条 / 十字 / 符号）、网点大小与最小网点、面积或直径正比的响应、网点增益、灰阶分级（2–32 档）、点融合（距离场 smooth-min，单独的小点不会被吃掉）、抗锯齿开关；符号网点按明暗从一串符号里挑（草图：点 → 斜线 → 十字 → 网格 → 点线；打字机：留白 → 横线 → 4 → 6 → % → 实心点 → 三角；线格；记号；自定义序列可用 `. / + # *` 简写），线粗按格子比例、交界混合、点缀圆圈三角、种子；网格横向 / 纵向中心距、角度、方格或交错排列、偏移、网格扰动（涟漪 / 波浪 / 流动 / 随机，把每颗点从格心推开，规则网格成了被水波推歪的网；强度以格为单位、波长、种子，采样跟着点走）；颜色双色（网点色 + 背景色）/ 原图色 / CMYK 四层分色叠印（青 15° 品 75° 黄 0° 黑 45°）；边缘抗锯齿，CPU 渲染
...
- 体验：内置预设 34 个（抖动 11、排线 9：Hatching、Pencil、Engraving、Crosshatch、Woodcut、Rain、Beads、Blueprint Lines、Brick；网点 14：默认、Poster、Newsprint、Comic、CMYK Print、Ink Blob、Line Screen、Mosaic、Triangles、Color Dots、Honeycomb、Symbol Sketch、Typewriter、Yellow Pop（涟漪扰动）、用户预设（按风格分页显示，「历史」页都在）、撤销重做与快捷键（切风格页签也可撤销）、浅 / 深主题、GPU（WebGL2）开关；默认方案是有序 Bayer 2×2 单色，每种风格记住上次用的方案
- 参数解读：停在任一参数标签上弹出「这是什么 / 各个值什么意思 / 什么时候选」，下拉里每个选项也逐条解释
- 界面：无应用内顶栏，macOS 上系统标题栏只留三个圆点、下面一条分隔线；左右分栏可拖拽、中间一条分隔线，左栏参数栅格按栏宽在一排三 / 二 / 一之间自适应；左栏页签「抖动 | 排线 | 网点 | 历史」，前三个就是艺术风格（切页签即切风格，各自只列自己的预设）；参数分节默认展开、可逐节收起（收起时显示当前值摘要），节间不画线，顺序是基础 → 笔画（仅排线）→ 网点（仅网点）→ 颜色 → 影调 → 网格（仅抖动）→ 特效，每节标题右端有「重置」退回当前方案的值；预览画布按宽度 7.2% 加圆角（只在预览，导出不带）；缩放低于 100% 时预览照浏览器 / Figma 显示导出图的方式（mipmap 逐级平均 + 双线性）缩到屏幕物理像素，看到的就是导出图缩到那么大的真实观感，100% 及以上每个像素都是实的（见 `docs/PREVIEW_SCALING.md`）；色块点开的取色层默认 HSB，可切 HEX；预设卡片最多三行、其余折起，「还原」与「保存预设」在左栏操作行（保存点开浮层，名称已预填）；选中态一律墨色描边，不用品牌橙；画布尺寸 / 适配 / 缩放集中在预览区右上角的「画布」菜单，导出按钮在它右侧：主按钮按素材类型在「导出图片」（存 PNG）与「导出视频」（开对话框）之间切，左边是「导出帧」（当前帧存成 SVG 矢量图），左栏不再放导出按钮；打开、复制当前帧 PNG、撤销、重做只走快捷键（⌘/Ctrl + O / C / Z / ⇧Z，左栏「设置」里有一览），面板上不放这些按钮
