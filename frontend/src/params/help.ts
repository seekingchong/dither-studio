...
export const PARAM_HELP: Readonly<Record<string, ParamHelp>> = {
  ...
  'tone.linear': {
    ...
    tip: '想还原老软件那种偏亮的味道就关掉它。排线按看起来的明暗定粗细，不走这一档。',
  },
  'tone.grayFormula': {
  ...
  'hatch.levels': {
    summary: '明暗分成几档，每档一种粗细。2 就只有粗细两种，越多过渡越顺。2–16，默认 6。',
  },
  'hatch.length': {
    summary: '每一笔多长。100% 刚好贯穿格子、首尾相接成线；小于 100 断成短划，大于 100 互相压住。',
    tip: '短划配圆角像针脚，100% 配「沿斜线」连线像铜版画。',
  },
  'hatch.maxWidth': {
    summary: '最暗一档的粗细，以相邻平行线的间距为 100%——到 100 就挨在一起成实色。默认 70。',
  },
  'hatch.minWidth': {
    summary: '最亮一档的粗细。0 是亮部留白；大于 0 亮部也有一根至少 1px 的细线。默认 8。',
    tip: '想要图里那种满铺的细线底纹就留一点，想要干净留白就归零。',
  },
  'hatch.roundness': {
    summary: '笔画两端的圆角。0 是方头，100 是完全圆头的胶囊形。默认 40。',
  },
  'hatch.cross': {
    summary: '暗部再叠一层垂直方向的线，越暗第二层越粗——素描里的交叉排线。',
  },
  'hatch.crossStart': {
    summary: '暗到几成才开始出现第二层线。0 全图都交叉，越高只有最暗处交叉。默认 50。',
  },
  'hatch.stagger': {
    summary: '奇数行相对偶数行左右错开多少，100 是错开一格。0 是整齐方阵，错开后像砖缝或蜂窝。',
  },
  'hatch.link': {
    summary: '要不要用一根细线把每一笔串起来。',
    options: {
      none: '不串，每一笔各是各的',
      stroke: '顺着笔画方向贯穿格子，笔画首尾接成连续的线',
      row: '每行一根横线穿过笔心，像珠子串在线上',
      col: '每列一根竖线穿过笔心',
      grid: '横竖都有，串成一张网',
    },
    tip: '「沿斜线」+ 长度 100% 就是铜版画那种粗细起伏的连续线。',
  },
  'hatch.linkWidth': {
    summary: '串线的粗细。1–16px，默认 1。细线只是暗示，粗了会和笔画抢主次。',
  },
  'hatch.linkColor': {
    summary: '串线的颜色，默认中灰。取前景色就融进笔画，取浅一档就是底纹。',
  },
  'hatch.ink': {
    summary: '笔画的颜色，默认近黑。',
  },
  'hatch.paper': {
    summary: '纸的颜色，默认浅灰。换成米色有纸本素描感，换深色配浅前景色就是反相。',
  },
  ...
  'halftone.holes': {
    summary: '暗处不再是越长越大的黑点，而是翻成一片黑底、在上面挖白孔：越暗孔越小，全黑处消失。',
    tip: '像素画海报那种点阵：亮处黑点、暗处白孔，黑区边缘一串圆鼓。',
  },
  'halftone.holeStart': {
    summary: '从多暗开始翻成白孔。5–95%，默认 50：中灰之前长黑点，之后挖白孔。',
    tip: '调低黑区更大更早出现，调高黑点一路长到相连才翻。',
  },
  'halftone.holeSize': {
    summary: '刚翻过来时白孔占格子多大，再往暗处孔逐渐缩小。0–100%，默认 50。',
    tip: '和网点大小差不多时，翻过去的那一瞬黑白比例最接近。',
  },
  'halftone.field': {
    summary: '白孔下面那块黑底占格子多大。越大相邻黑底越连成片，黑区边缘越圆鼓。100–200%，默认 150。',
    tip: '低于 142 四角会留缝，露出一串小白星。',
  },
  'halftone.antialias': {
