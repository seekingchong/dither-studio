...
export const PARAM_HELP: Readonly<Record<string, ParamHelp>> = {
  ...
  'halftone.shape': {
    ...
    options: {
      ...
      line: '横条，粗细随明暗变化的线网，一格一段',
      smoothline: '横条，粗细顺滑起伏，不在格子边界打台阶',
      cross: '十字，像刺绣与织物',
    },
...
export const EFFECT_HELP: Readonly<Record<string, ParamHelp>> = {
  ...
  sourceOverlay: {
    summary: '把原图（视频就是当前帧）夹在成品的底色与前景之间：底色上铺原图，墨点 / 笔画 / 网点 / 符号留在最上面。',
    tip: '默认「正常」100% 就是照片上印着网点，调低不透明度底色透出来；要原图盖在整张成品上就把「层级」切到「最上层」。缩放绕画布中心，偏移按画布尺寸的百分比。',
  },
  levelOutline: {
  ...
  blocks: {
    summary: '在成品上按网格随机撒几块色块，可带文字，像界面出错时掉出来的碎片。',
    tip: '尺寸以当前像素尺寸（格子）为单位，块永远对齐网格；每块的颜色与文字都能单独改。',
  },
};
