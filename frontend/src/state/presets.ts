...
export const BUILTIN_PRESETS: BuiltinPreset[] = [
  ...
  // 符号大小 122%：圆角方的半边是 0.85r，1.22 × 0.85 > 1 抵过格子边，成片的黑连起来只剩发丝细的纸色缝，黑块上就浮出一层网格；
  // 同一个 122% 下圆点会糊成一片，所以亮部缩小拉满到 90%——最暗的圆角方仍是 122%，圆点落到 104%（刚好一格宽、邻格相切），
  // 三角 85%、十字 67%、小点 31%（13px 的格子里只剩两三像素），一眼就能把六种符号的层级分开。
  // 断斜线是跨格图元，长度只跟格子走、不受大小影响；交界混合 58% 让相邻两阶掺在一起，边界像手排的一样散开。
  {
    id: 'glyph-atlas',
    name: 'Ink Atlas',
    hint: '暖白纸配墨蓝：小点、断斜线、十字、三角、圆点到圆角方，黑块里透出细网格',
    params: {
      'style.type': 'glyph',
      'glyph.ramp': 'custom',
      'glyph.levels': 7,
      ...glyphShapes(['blank', 'pip', 'slashdash', 'plus', 'tri', 'dot', 'roundsquare']),
      'glyph.size': 122,
      'glyph.taper': 90,
      'glyph.stroke': 12,
      'glyph.mix': 58,
  ...
  // 浅纸深墨，不反相
  {
    ...
    params: {
      ...
      'tile.pitchY': 11,
      'glyph.colorMode': 'levels',
      ...glyphColors(['#4B42F2', '#4B42F2', '#3D34E8', '#332AD9', '#2A21C9', '#2019B2', '#171099', '#120C86']),
      'glyph.paper': '#F0EEF7',
      'tone.contrast': 15,
    },
    exposes: GL,
  },
  // 参考图：瑞士风格的双色方块海报——整幅画就是一张方格网，暗处的格子填满、与邻格连成整片实色，
