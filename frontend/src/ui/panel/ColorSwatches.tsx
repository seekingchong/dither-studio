...
import { GLYPH_MAX_LEVELS, PALETTE_PRESETS, buildLevelPalette, clampLevels, getPresetPalette, parseColorList, resolvePalette, rgbToHex, type ColorMode } from '@/engine';
import { getParamDef, glyphColorId, styleOf, type ParamValue } from '@/params';
...
const GLYPH_INK_ID = 'glyph.ink';
const GLYPH_PAPER_ID = 'glyph.paper';
const GLYPH_LEVELS = Array.from({ length: GLYPH_MAX_LEVELS }, (_, i) => i + 1);
 ...
 * 符号风格统一色下是符号颜色与背景色，分级配色下从亮到暗一阶一块再加背景色，原图色下只剩背景色。
 */
export function ColorSwatches() {
  const { style, ink, paper, dot, htPaper, inkMode, glyphMode, glyphInk, glyphPaper, glyphLevels, glyphColorsText, mode, levels, dark, light, stopsText, preset, customText, linear, setParam, setParams } =
    useStudioStore(
      useShallow((s) => ({
        ...
        glyphMode: String(s.params['glyph.colorMode']),
        glyphInk: String(s.params[GLYPH_INK_ID]),
        glyphPaper: String(s.params[GLYPH_PAPER_ID]),
        glyphLevels: clampLevels(Number(s.params['glyph.levels'])),
        glyphColorsText: GLYPH_LEVELS.map((k) => String(s.params[glyphColorId(k)])).join(' '),
  ...
  const model = useMemo(() => {
    ...
    if (style === 'glyph') {
      const colors = glyphMode === 'mono' ? [glyphInk, glyphPaper] : glyphMode === 'levels' ? [...glyphColorsText.split(' ').slice(0, glyphLevels), glyphPaper] : [glyphPaper];
      return { kind: 'glyph' as const, colors, presetLabel: '' };
    }
  ...
  }, [style, ink, paper, dot, htPaper, inkMode, glyphMode, glyphInk, glyphPaper, glyphLevels, glyphColorsText, mode, levels, dark, light, stops, preset, customText, linear]);
  ...
  const titleOf = (i: number) => {
    ...
    if (model?.kind === 'glyph') return i === count - 1 ? '背景色' : glyphMode === 'mono' ? '符号颜色' : `第 ${i + 1} 阶`;
  ...
  const applyColor = (i: number, hex: string) => {
    ...
    if (model.kind === 'glyph') return setParam(i === count - 1 ? GLYPH_PAPER_ID : glyphMode === 'mono' ? GLYPH_INK_ID : glyphColorId(i + 1), hex);
